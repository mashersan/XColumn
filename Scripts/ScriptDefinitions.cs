...
namespace XColumn.Scripts
{
    /// <summary>
    /// アプリケーションでWebViewに注入するJavaScriptコードやCSS定義を一元管理する静的クラス。
    /// 各定数は注入用の生コードであり、内容を変更するとページ挙動に直結するため取り扱いに注意してください。
    /// </summary>
    public static class ScriptDefinitions
    {
        #region CSS Definitions

        /// <summary>メニュー（ヘッダーバナー）を非表示にするCSS定義。</summary>
        ...
        /// <summary>リストヘッダー（編集・メンバー・フォロワー等の見出し領域）を非表示にするCSS定義。</summary>
        ...
        /// <summary>キー入力検知スクリプト（ESCキー対応。ダイアログ表示中はX標準動作に委ねる）。</summary>
        ...
        /// <summary>リプライ検出スクリプト（返信ツイートのセルに xcolumn-is-reply クラスを付与）。</summary>
        ...
        /// <summary>入力監視スクリプト（IME対応。入力中状態を inputState メッセージで通知）。</summary>
        ...
        /// <summary>トレンド/検索リンクのクリックを横取りし、新規カラムでの検索を要求するスクリプト。</summary>
        ...
        /// <summary>横スクロール同期スクリプト（Shift+Wheel / 横ホイールを horizontalScroll で通知）。</summary>
        ...
        /// <summary>
        /// スクロール状態（トップにいるか）をC#に通知する固定許容誤差(1px)版スクリプト。
        /// 許容誤差を設定で変えたい場合は <see cref="GetScrollStateNotifierScript(int)"/> を使用してください。
        /// </summary>
        ...
        /// 設定で指定された許容誤差(px)をスクリプト内に埋め込み、実行時にも window 変数経由で参照します。
        /// </summary>
        /// <param name="tolerance">トップ判定の許容誤差（ピクセル）。</param>
    ...
    (function() {
        ...
        /// <summary>サイドバー/プロフィール情報から自分のリストページ(/lists)へ遷移するスクリプト。</summary>
        ...
        /// <summary>動画の自動再生を強力にブロックするスクリプト（直近のユーザー操作がない play() を拒否）。</summary>
        ...
        /// <summary>時刻表示を相対時刻から絶対時刻(yyyy/MM/dd HH:mm)へ変換するスクリプト（即時反映・定期実行・OFF時復元対応）。</summary>
        ...
        /// <summary>
        /// 指定CSSをページに注入するJavaScriptコードを生成します。
        /// </summary>
        /// <param name="cssToInject">注入するCSS文字列。</param>
        public static string GetCssInjectionScript(string cssToInject)
        {
            if (string.IsNullOrEmpty(cssToInject)) return "";

            // テンプレートリテラルへ安全に埋め込むためのエスケープ処理（バックスラッシュ・バッククォート・改行）
        ...
        /// </summary>
        /// <param name="ngWords">NGワードのリスト。</param>
