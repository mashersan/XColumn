...
## 🛠️ その他の便利機能
* **休止モード（💤）＆ API制限時の自動休止**: カラム上部の💤ボタンで、重いページを一時的に軽量な待機画面へ置き換えてメモリを解放できます。さらに X のAPI制限（HTTP 429）を検知すると該当カラムを自動で休止モードへ切り替え、制限中の無駄なリクエストを抑制します。復帰は💤ボタンの再クリックで行えます。（この自動休止は設定でOFFにできます）
...
### ダウンロード
* [v1.47.0(2026.06.17)](https://github.com/mashersan/XColumn/releases)
### 基本操作
...
* **Suspend mode (💤) & auto-suspend on rate limit**: Use the 💤 button at the top of a column to temporarily replace heavy pages with a lightweight standby screen and free up memory. In addition, when an API rate limit (HTTP 429) is detected, the column automatically switches to Suspend mode to suppress unnecessary requests during the limit. Click the 💤 button again to resume. (This auto-suspend can be turned off in Settings.)
