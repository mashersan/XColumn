...
* **カラムごとの自動更新**:
    ...
    * **（v1.4.2）** アプリがアクティブ（操作中）の時に自動更新を一時停止するかどうかを設定で切り替えられます。
* **複数アカウント対応（プロファイル機能）**:
...
7.  画面上部の「アクティブ時停止」チェックボックスで、操作中の自動更新停止の有無を切り替えられます。
8.  各カラムのヘッダーにあるボタンで、以下の操作が可能です。
    * **↻**: そのカラムを手動で更新します。
...
- **v1.4.2 (2025/11/17)**
  - アプリがアクティブ（操作中）の時に自動更新を一時停止するかどうかを選択できる設定（チェックボックス）を追加
