...
![XColumn Screenshot](http://masherhouse.com/wp-content/uploads/2025/11/TweetDeck_image.jpg) 
...
* **カラムごとの自動更新**:
    ...
    * **（v1.4.0）** アプリがアクティブ（操作中）の時は自動更新を一時停止し、バックグラウンドに回ると再開します。
* **複数アカウント対応（プロファイル機能）**:
    * **（v1.4.0）** 画面上部の「プロファイル」メニューから、複数の設定（カラム構成、ウィンドウ位置、ログインアカウント）を切り替えることができます。
