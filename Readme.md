...
## 🛠️ その他の便利機能
* **休止モード（💤）＆ API制限時の自動休止**: カラム上部の💤ボタンで、重いページを一時的に軽量な待機画面へ置き換えてメモリを解放できます。さらに X のAPI制限（HTTP 429）を検知すると該当カラムを自動で休止モードへ切り替え、制限中の無駄なリクエストを抑制します。復帰は💤ボタンの再クリックで行えます。
...
### ダウンロード
* [v1.45.0(2026.06.15)](https://github.com/mashersan/XColumn/releases)
### 基本操作
