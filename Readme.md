...
## 🛠️ その他の便利機能
* **キーボードショートカット**: 矢印キーでのカラム移動、数字キーでのジャンプ、PageUp/Downでのスクロール操作に対応。
...
5.  XColumnを起動し、上部メニューの **「ツール」→「拡張機能の管理...」** を開きます。
...
- **v1.17.0 (2025/12/05)**
  ...
  - **U改善**: 設定項目をタブ（表示、カラム、動作、カスタムCSS）に分類し、レイアウトを見やすく整理しました。
  - **操作性改善**: クリックによるカラム選択・フォーカス移動機能を追加し、レイアウト崩れを修正しました。
- **v1.16.0 (2025/12/02)**
  - **機能追加**: キーボードショートカットによるナビゲーション昨日の追加
    - 矢印キー（←→）: カラムのフォーカス移動
